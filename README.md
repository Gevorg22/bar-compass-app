...
Проект покрыт юнит-тестами на **Vitest** с окружением **jsdom**.

Тесты охватывают:

- `shared/lib/geo` — формулы расстояния (Haversine), азимут, форматирование дистанции, нормализация угла
- `shared/lib/openingHours` — парсинг часов работы, режим 24/7, диапазоны дней, ночные смены, маркер off
- `shared/lib/cache` — чтение и запись в sessionStorage, проверка TTL, инвалидация по времени
- `shared/lib/cn` — объединение классов с фильтрацией falsy-значений
- `entities/place/lib/formatters` — метки типов заведений, эмодзи, получение названия места
